...
for i in range(500):
    ...
    dw,dw_0 = backward(train_data_arr,y_i,w,w_0)
    dw = dw.reshape(196,1)
    dw = dw + lamda*w
    dw_0 = dw_0 + lamda*w_0
    w = w - eta*dw
...
y = np.ones([val_target_arr.shape[0]])
one = np.where(val_target_arr==1)
y[one] = -1
y = y.reshape(1,val_target_arr.shape[0])


# In[598]:


val_loss = logistic_regression(val_data_arr,y,w,w_0)
